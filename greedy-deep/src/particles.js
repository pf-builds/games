// Greedy Deep — pooled particles + floaters.
//
// COPIED VERBATIM from peasant-swarm/src/particles.js (Pool, PS.Particles with
// burst/smoke/ring/update/draw, and PS.Floaters), renamed onto window.GDParticles so
// the two games can sit in the same arcade without colliding. R4 6 lists this copy as
// M2/M3 setup; the juice pass that actually spawns from it is M4 (PRD 14), so this
// file is staged and deliberately not wired into the render loop yet.
(function () {
  const PS = (window.GDParticles = window.GDParticles || {});
  ...
  PS.Particles = function (cap) {
    ...
    return {
      // square pixel chunks that arc and drop (rock chips, ore fragments, cart spill)
      ...
      // slow grey puff drifting up (lamp smoke, gas pocket)
      smoke(x, y) {
        const p = pool.spawn();
      ...
      // expanding ring (band break, crit strike)
    ...
    return {
      ...
      draw(ctx) {
        ...
        for (const f of list) {
          ...
          ctx.font = "800 " + sz + "px ui-monospace, Menlo, monospace";
