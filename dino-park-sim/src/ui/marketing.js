...
export function campaignLadder() {
  ...
  return h('div', {},
    ...
    h('div', { class: 'cards grid-3 ladder-cards' }, ladder.map(({ def: c, lock, owned }, i) => h('div', { class: `card ${lock ? (owned ? 'owned' : 'locked') : ''}` },
      ...
      // Auto-renew governs FUTURE renewals, so its toggle stays available even while the campaign is running
      // (lock === 'already running'); otherwise an always-renewing campaign could never be switched off.
      c.kind !== 'perk' && c.kind !== 'memberships' ? autoRenewToggle(c) : null))));
}
