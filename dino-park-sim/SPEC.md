...
## Phase 2 — Difficulty modes & balance sweep (2026-09-12)

**1. Difficulty modes (`data/difficulty.json`).** Three complete parameter sets, applied as multipliers/overrides on top of balance.json so **Standard stays exactly as tuned**:
- **Relaxed** — build-and-enjoy: start loan ~$40k, APR 2%, grace 6 quarters, `foreclosure_enabled: false` (overdraft interest still accrues and the Bank warns, but the park never closes), attendance ×1.2, event frequency ×0.7, food/salaries ×0.9, breakout chance ×0.5.
- **Standard** — current values, unchanged.
- **Classic** — the original's shoestring, scaled to our economy: start loan ~$10k, APR 10%, grace 1 quarter, debt cap base low, attendance ×0.85, event frequency ×1.2, food/salaries ×1.15, breakout chance ×1.25, no first-year morale softening.
Selection: a New Game dialog (from Settings and from the foreclosure/restart screen) with three cards (name, one-line feel, key numbers). Mode is fixed for the run, stored in state, shown as a chip in the status bar tooltip, in Reports, on the Goals panel and the Year-5 Report Card (Classic earns a "Classic" badge on Grand Park/Report Card). Default remains Standard. Old saves keep Standard.
**Acceptance (simulator, `tools/sim.html`, 5 seeds each):** classic-opening path → Relaxed ends year 1 clearly positive; Standard positive (as today); Classic forecloses or ends year 1 negative. A new **"Classic, played well"** scenario (starter herbivore + guide + seeds + $5 ticket + flyers, second starter dino in Q2, no marquee) → survives year 1 on Classic. Slow-bleed path → forecloses on Standard and Classic; on Relaxed never forecloses (Bank shows the warning state instead).

**2. Balance sweep (data only).** Add scenarios to `tools/sim-driver.js` and tune `data/*.json` until the targets hold on Standard; report a before/after table:
- (e) **Facility ROI:** every facility tier pays back its cost within 6 quarters on a "good run" park (3 pens, 4 dinos, staff, $6 ticket). Retune tier costs/effects if not.
- (f) **Prize pacing:** on the good run, spend prizes land roughly one per 1–2 quarters; all 8 reachable by end of year 4; the last one not before year 3.
- (g) **Memberships:** over 8 quarters at a good rating, member revenue is a meaningful but not dominant line (15–35% of revenue); at a poor rating (starved, no staff) members churn to near zero within 3 quarters.
- (h) **Campaign ROI:** no campaign has negative expected ROI at the point it unlocks; social stacking does not out-earn TV per dollar by more than 2×.
- (i) **Breakouts per year:** adequate fence → 0; small species one tier short → ≤1; large carnivore in wood → ≥1 per quarter; electrified + maintenance holds every species.
- (j) **Events:** ≤3 negative events per quarter on Standard for a mid park; positives ≥1 per quarter.
- (k) **Seeds:** marsh ≥40% / plains ≥25% / desert ≥10% plant-cost saving (re-verify after any food retune).

**3. M5 minors to close:** tip toasts must not stack over an open Goals panel (defer while any panel/modal is open) and the clock pauses under Goals like other panels; dinosaurs bought outside the Park view appear immediately with the truck animation only when the Park view is visible (queue drains instantly off-screen); at the year-5 close the Report Card opens AFTER the Quarterly Report is dismissed, never under it; Settings auto-restock table fits at both sizes; vegetation readout grammar fixed and not repeated; `DPS.living.forceEscape()` drives the siren + shake; master volume observable in `DPS.audioState()`.

**Acceptance (critics):** New Game dialog offers the three modes with correct numbers; a run's mode is fixed and shown where specified; all simulator scenarios in (1) and (2) pass with the reported numbers; the six M5 minors closed; zero console errors; M1–M5 regressions clean (parcels/biome, seeds, auto-restock, campaigns, memberships, prizes, goals, report card, sound, save/load incl. mode).

## Phase 2b — Economy correction (2026-09-12)

Two findings from the Phase 2 sweep, both accepted by Peter for correction. The Phase 2 builder proved by grid search that neither is reachable by tuning the existing dials, so each needs a new mechanic. **Standard's early game stays as playtested — do not touch the starter economy's first four quarters on Standard.**

**1. Fixed quarterly overhead (makes Classic actually brutal).** New `balance.overhead`: a per-quarter charge that scales with **land held**, not revenue — a park licence / property tax of `rate_per_tile × owned parcel tiles`, plus a flat `base` per quarter. Charged at quarter close as its own ledger line ("Property tax"), shown in the Quarterly Report and Reports, with a tooltip teaching fixed vs variable cost. Per-mode multiplier in `data/difficulty.json`: Relaxed 0 (or near), Standard low enough that the playtested early game is unchanged, Classic high enough to bite. The point: holding land costs money whether or not it earns, so idle or over-bought land hurts.
- **Acceptance:** on Classic the bare classic opening ends year 1 **negative or foreclosed** on ≥4 of 5 seeds, while "Classic, played well" still survives year 1 on ≥4 of 5. On Standard, the classic opening's year-1 cash is within **±10%** of its current value (quote before/after). On Relaxed, no meaningful change.

**2. Concession saturation (kills the late-game money printer).** Per-visitor concession spend must **saturate** instead of scaling linearly with store tiers and attendance: replace the current linear term with a diminishing-returns curve (e.g. `spend = cap × tierFactor / (1 + k / attendance_scale)` or an explicit per-tier cap table in `balance.revenue`), so each store tier adds less than the last and a huge park cannot compound past a ceiling. Same treatment for any other line found to compound without limit during the sweep (check memberships and appeal→attendance).
- **Acceptance (new sim scenario "max-growth reinvestor": always reinvest all cash into the highest-return option, 5 seeds, 5 years):** net worth at end of year 4 lands in a **defined band under $1.5M** (down from ~$17M) and grows sub-exponentially year over year; **all eight spend prizes are not earned before year 3**; the good-run park's revenue mix stays sane (concessions ≤ 50% of revenue). Standard's good-run park (3 pens, 4 staff, $6 ticket) must stay within **±10%** of its current year-1 and year-2 cash.

**3. Re-verify after retune:** every Phase 2 balance target (e)–(k) and the difficulty scenarios must still hold, plus the Phase 1 economy scenarios (classic opening survives year 1 on Standard; slow bleed forecloses on Standard and Classic). Report a full before/after table.

## Phase 4 B1 — `DPS.selfTest()` + crowd flow (2026-09-23)

**1. `DPS.selfTest()`** (`src/main.js`): a fast regression pass for critics and the console. Synchronous, ~60–110 ms, returns `{ pass, fail, ms, results: [{ name, ok, detail }] }`. Every test runs on a **throwaway park** made quiet (`sfx`/`ambient` off, `reduce_motion` on, autosave off) and the player's game is put back exactly: the same state object, clock speed, save slot (`localStorage`), and only the modals/toasts the tests opened are closed. The last result compares the state JSON before and after. The Living Park crowd is rebuilt from state afterwards (agents are visual only; it refills in a few seconds). Tests (20 results):
- **boot**: all `data/*.json` files loaded; every species has a loaded sprite or a flagged fallback (`spriteStatus()` in `render/living.js`: loaded / fallback / loading); the live state's cash and debt are finite numbers.
- **economy**: buying the smallest parcel (as its cheapest-species' preferred biome), its fence and that species each move cash by exactly the listed price (`parcelPrice`, `fenceCost`, `shop_price`); `advanceDays(90)` fires a quarter report without throwing; cash and debt stay finite.
- **marketing** (regression for the 2026-09-21 bug): a running campaign shows its auto-renew toggle in the Full Marketing card AND in the toolbar dropdown; clicking on then off leaves `auto_renew[id] === false`; the campaign does not renew when it ends; a control run left on does renew. (Verified: re-introducing the old `!lock` condition makes 4 of these fail.)
- **living** (on the title demo park, 120 visitors): agent counts within `balance.living` caps (visitors ≤ `max_sprites`, litter ≤ `litter_max`, cars ≤ lot slots, staff and dinosaurs match state); no NaN position after `selftest.living_ticks` (300) ticks.
- **crowd**: averaged `crowdStats()` over `selftest.crowd_frames` frames must meet `crowd_centre_share_max` (0.30), and nobody is off the paving.
Support hooks: `tick(force)` runs agents under an open modal (selfTest only), `modalDepth()`/`closeAbove(n)` in `ui/modals.js`, `buildDemoPark({ settings })` in `ui/title.js`, `dataFiles()` in `state.js`.

**2. Crowd flow.** Metric `DPS.living.crowd()` (`crowdStats()` in `sim/agents.js`), sampled at the drawn position of every visitor walking inside the fence on a straight stretch (≥ `crowd_junction_skip` from any junction): `centre_share` (within `crowd_centre_band` = 0.05 tiles of the centreline), `spine_centre_share`, `lateral_rms` (fraction of paving half-width), `overlap_share`/`dwell_overlap_share` (another figure within `crowd_overlap`), and `off_path` (any non-panicking visitor off the paving, in the fountain or inside a forecourt building; must be 0). Fix: each walker keeps a persistent lane (`lane_jitter`, `lane_drift`, `lane_margin`), applied to the route's entry point and to BOTH axes at a corner; per-route edge-cost jitter (`route_jitter`) spreads near-equal routes; gate arrivals use the walker's lane; dwell points stop `lane_margin` short of the paving edge (`dwell_toward`); tour-group followers are held on the path. Result on the 120-visitor demo park: centre share 0.49 → 0.22, spine 0.47 → 0.20, off-path samples ~10 per run → 0. Numbers and root cause: `game-research/ff-phase4-b1-notes.md`.
