...
## Parked during Phase 2 (difficulty modes + balance sweep, 2026-09-12)
- **Classic's bare classic opening cannot end year 1 negative with the SPEC's Classic dials.** One starter herbivore earns about four times its running costs on Standard ($54k revenue against ~$22k of costs in year 1); attendance ×0.85 and food/wages ×1.15 leave it at +$29k–41k, and even ×0.5 visitors / ×2 wages / ×2 feed leaves +$6k–24k because the tour guide (the only fixed cost) quits after a walkout event and a lone herbivore always covers its own feed. Making the bare opening fail while "played well" survives needs a window the multiplicative dials do not have (the second starter adds ~45% revenue but nearly as much cost). Options for a real shoestring: a Classic-only fixed quarterly overhead (park licence / property tax), a Classic reference ticket below $5, or a smaller appeal from the cleanliness term. The sim row stays FAIL on purpose until the design picks one.
- Facility ROI is now measured on two reference parks (GOOD: 3 pens / 4 dinosaurs; BIG: 6 pens / 12 dinosaurs, 9 staff) because capacity and office tiers only bind on a big park. The office's value (event mitigation on a park where one closed day costs $16k) is so noisy that it needs 20 seeds; a deterministic "events averted" counter on the office would make it legible in-game too.
- Prize pacing (one per 1–2 quarters, last in year 3) holds for a two-purchases-per-quarter good run; a max-growth reinvestor earns all eight spend prizes inside year 1 because the Standard economy returns >100% a quarter on concessions. If that matters, prizes could require a minimum park age or rating alongside the spend threshold.
- The Restrooms and Vet Clinic tiers now carry a small appeal_bonus so they pay back at a fixed $6 ticket; satisfaction alone is only worth money above the reference ticket. A satisfaction → word-of-mouth (reputation) link would be the more honest mechanic.
- The random escape events now need a minimum daily breakout chance (events.json `requires_breakout_risk: 0.005`); a low-danger species one tier short escapes ~1/year on its own roll. Escapes still zero the fence condition after a few hits, which does nothing for an under-tier pen; a "fence collapses to the tier below" rule would give condition a meaning there.
- Relaxed keeps the same debt cap base as Standard so the Bank warning has teeth; a Relaxed-only "bank letter" modal (once, the first time the cap is crossed) would teach the warning better than the ticker + Bank banner.
- The New Game dialog shows every mode's numbers; a "custom" fourth card (per-dial sliders, stored in the save) would be a small step from here since every dial is already a per-run parameter.

## Parked during Phase 4 B1 (selfTest + crowd flow, 2026-09-23)
- Plaza furniture sits on the plaza walkway ring: the four benches (0.9 tiles out on the axes) straddle the ring centreline at the spine and cross-path junctions, and the diagonal planters sit on the ring corners, so walkers pass over them. The Carousel prize (7.35, 3.65, r 0.32) covers the plaza_n/plaza_e corner path. Either move the furniture inside the ring (and the carousel off the corner) or route the plaza ring at 1.25 tiles out.
- The plaza point of interest (weight 4) uses spotsForRect, which lands its only dwell spots on the four junction points (the benches). A ring of standing spots between the fountain and the ring, clear of the props, would make it a real gathering place.
- Staff still walk through the office strip: the vet heading home and new staff spawning at OFFICE_DOOR (the south face) cross the building from the office_path on its north face (the M3 "door side per facility" item). About 0.1% of staff samples in a busy park. A door on the office_path side, or a spur round the building, would close it.
- Panicking visitors (escape scatter) run in a straight line away from the loose animal and can cross pens and grass; the selfTest's off-path check excludes them. A scatter that flees along the walkway graph would keep them on the paths.
- The crowd metric counts visitors only. Adding staff to `off_path` (once the office door is fixed) would make the bounds check cover everyone.
- Route jitter spreads near-equal routes (plaza west/east split 61/39 → 51/49), but the gate is a single entrance, so ~12 of ~50 walking visitors are on the spine at any time. A second gate or a side entrance would thin the spine for real.

## Parked during Phase 4 B2 (cars, plaza props, dino motion, sprite cleanup, 2026-09-23)
- Enclosure fences (`drawWall`) were not restyled (optional in the brief, out of time budget). They are the last plaza-adjacent prop still in the pre-B2 flat style.
- More baked sprite smudges: the stegosaurus carries a visible grey under-belly shadow (see the B2 herd shot) and the hadrosaurus has a baked white pocket between its legs (29 near-white px) plus light halo pixels on its outline. `tools/process-sprites.py` `CAST_SHADOW` is opt-in per species, so adding a species there (and re-checking it by eye) is the fix. The shadow-tone band score is only meaningful where the body colour differs from the backdrop: triceratops scores 507/507 because its body is backdrop-toned, so the number alone is not evidence.
- `tools/process-sprites.py` `remove_bg`: `px - pv` is int16, so `(px - pv) ** 2` overflows for channel gaps over 181 and the step test silently fails (the "invalid value in sqrt" warning). Fixing it would change the other sprites' outputs, so it needs a full re-check of all 24 when someone takes it on.
- Title screen: `render()` only flushes the label queue when not in title mode, but facilities, the gate, the lot sign and the tram stop queue their tags every frame regardless, so `labelQueue` grows by ~20 closures a frame on the title backdrop. One line (`else labelQueue.length = 0`) fixes it.
- The title backdrop runs with the clock at 0, so its dinosaurs, cars and crowd stand still (only the camera pans). Running the demo park at 1x would show off the B2 motion on the landing screen.
...
## Parked after Phase 4 (Peter, 2026-09-23)
- Title screen: three Park Prize toasts (Lamp Posts, Dinosaur Statues, Fountain: Stone Basin) stack over the landing screen on a cold load. Pre-existing, not a Phase 4 regression (same on the pre-phase tree `68f8a2d`). Likely path: `buildDemoPark` in `src/ui/title.js` spends the showcase park past every prize threshold, `prizes.js` `digestAdd`s each award, and the toast layer shows the last few. Fix ideas: skip the digest while `#app.title-mode` is on (or award the demo park's prizes state-only), and clear the toast queue when the title tears down.
